import { z } from "zod";
import { VehicleType, VehicleCapacityType, CapacityUnit, VehicleStatus } from "@prisma/client";
...
export const createVehicleSchema = z.object({
  ...
  acquisitionCost: z.union([z.number(), z.string()]).transform((val) => String(val)),
  ...
  maximumCapacity: z.union([z.number(), z.string()]).transform((val) => String(val)),
  capacityUnit: z.nativeEnum(CapacityUnit),
  odometerReading: z.union([z.number(), z.string()]).transform((val) => String(val)),
