...
class DriversView extends GetView<DashboardController> {
          ...
          DashboardCard(
            ...
            child: Row(
              children: <Widget>[
                ...
                Expanded(
                  child: TextFormField(
                    decoration: const InputDecoration(
                      labelText: 'Search by Name or License Number',
                      prefixIcon: Icon(Icons.search),
                      contentPadding: EdgeInsets.symmetric(horizontal: 12, vertical: 8),
                    ),
                    onChanged: (String value) {
                      controller.driverSearchQuery.value = value;
                    },
                  ),
                ),
                ...
                SizedBox(
                  width: 200,
                  child: Obx(() => DropdownButtonFormField<DriverStatus?>(
                        value: controller.selectedDriverStatusFilter.value,
                        decoration: const InputDecoration(
                          labelText: 'Filter by Status',
                          contentPadding: EdgeInsets.symmetric(horizontal: 12, vertical: 8),
                        ),
                        items: <DropdownMenuItem<DriverStatus?>>[
                        ...
                        onChanged: (DriverStatus? status) {
                          controller.selectedDriverStatusFilter.value = status;
                        },
                      )),
                ),
              ],
                            ...
                            return DataRow(cells: <DataCell>[
                              ...
                              DataCell(Column(
                                ...
                                children: <Widget>[
                                  ...
                                  Text(
                                    '${item.email} • ${item.contactNumber} • Code: ${item.employeeCode}',
                              ...
                              DataCell(Column(
                                ...
                                children: <Widget>[
                                  ...
                                  const SizedBox(height: 2),
                                  Text(
                                    'Cat: ${item.licenseCategory.name} • Exp: ${item.licenseExpiryDate.year}-${item.licenseExpiryDate.month.toString().padLeft(2, '0')}-${item.licenseExpiryDate.day.toString().padLeft(2, '0')}',
                              ...
                              DataCell(Row(
                                ...
                                children: <Widget>[
                                  IconButton(
                                    ...
                                    onPressed: () => _showEditDriverDialog(context, item),
                                  ),
                                ],
                              )),
                            ]);
  ...
  void _showRegisterDriverDialog(BuildContext context) {
    ...
    final TextEditingController employeeCodeCtrl = TextEditingController();
    ...
    DateTime selectedIssueDate = DateTime.now().subtract(const Duration(days: 365));
                ...
                return Form(
                  ...
                  child: Column(
                      ...
                      children: <Widget>[
                        ...
                        Text('Enter the driver\'s license number to verify if their profile exists in our database.',
                            style: TextStyle(fontSize: 12, color: theme.colorScheme.onSurface.withOpacity(0.6))),
                        const SizedBox(height: 16),
                        TextFormField(
                            controller: licenseCheckCtrl,
                            decoration: const InputDecoration(labelText: 'Driver License Number', hintText: 'e.g. DL-1420230012345'),
              ...
              return Form(
                ...
                child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[
                  isExisting.value
                      ? Container(
                          ...
                          child: Row(children: <Widget>[
                            ...
                            Expanded(
                                child: Text(
                                    'License "${licenseCheckCtrl.text.trim()}" found in database. Auto-filling existing driver profile.',
                  ...
                  Row(children: <Widget>[
                    ...
                    const SizedBox(width: 8),
                    const Text('2. Driver Profile Details', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13))
                  ]),
                  const Divider(height: 16),
                  TextFormField(
                      ...
                      return Column(children: <Widget>[
                        DropdownButtonFormField<LicenseCategory>(
                            ...
                            items: LicenseCategory.values
                                .map((LicenseCategory c) => DropdownMenuItem<LicenseCategory>(value: c, child: Text(c.name)))
                                .toList(),
                        ...
                        TextFormField(
                            ...
                            readOnly: true,
                            decoration: const InputDecoration(labelText: 'License Issue Date', suffixIcon: Icon(Icons.calendar_today)),
                            onTap: () async {
                        ...
                        TextFormField(
                            ...
                            readOnly: true,
                            decoration: const InputDecoration(labelText: 'License Expiry Date', suffixIcon: Icon(Icons.calendar_today)),
                            onTap: () async {
                              final DateTime? date = await showDatePicker(
                                  context: context,
              ...
              onPressed: () {
                if (regFormKey.currentState?.validate() ?? false) {
                  final DriverModel driverData = DriverModel(
                    ...
                    licenseIssuedAt: selectedIssueDate,
  ...
  void _showEditDriverDialog(BuildContext context, DriverModel current) {
    final ThemeData theme = Theme.of(context);
    ...
    final TextEditingController employeeCodeCtrl = TextEditingController(text: current.employeeCode);
    final TextEditingController issueDateCtrl = TextEditingController(
        text:
            '${current.licenseIssuedAt.year}-${current.licenseIssuedAt.month.toString().padLeft(2, '0')}-${current.licenseIssuedAt.day.toString().padLeft(2, '0')}');
    final TextEditingController expiryCtrl = TextEditingController(
        text:
            '${current.licenseExpiryDate.year}-${current.licenseExpiryDate.month.toString().padLeft(2, '0')}-${current.licenseExpiryDate.day.toString().padLeft(2, '0')}');
    ...
    DateTime selectedIssueDate = current.licenseIssuedAt;
    DateTime selectedExpiryDate = current.licenseExpiryDate;
